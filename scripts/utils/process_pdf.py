...
def load_citation_metadata(pdf_path):
    ...
    try:
        ...
        if authors:
            ...
            author_orcid = first_author.get('orcid', '')
...
def embed_metadata(pdf_path, metadata):
    ...
    try:
        ...
        raise RuntimeError(f"Exiftool metadata embedding failed: {stderr_msg}")
...
def build_metadata_dict(meta):
    ...
    if meta.get('author_orcid'):
        metadata['XMP-dc:CreatorTool'] = meta['author_orcid']
    if meta['url']:
